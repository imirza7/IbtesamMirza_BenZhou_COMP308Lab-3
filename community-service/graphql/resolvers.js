...
// Define the resolvers for the Community Post and Help Request operations
const resolvers = {
    Query: {
        // Get all community posts
        getAllCommunityPosts: async () => {
            try {
                return await CommunityPost.find().populate('author', 'username'); // Populate author with username
            } catch (err) {
        ...
        getCommunityPost: async (_, { id }) => {
            try {
                return await CommunityPost.findById(id).populate('author', 'username');
            } catch (err) {
        ...
        getAllHelpRequests: async () => {
            try {
                return await HelpRequest.find().populate('author', 'username').populate('volunteers', 'username');
            } catch (err) {
        ...
        getHelpRequest: async (_, { id }) => {
            try {
                return await HelpRequest.findById(id).populate('author', 'username').populate('volunteers', 'username');
            } catch (err) {
